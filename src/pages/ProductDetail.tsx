    ...
    return (
      <div className="relative min-h-screen">
        ...
        <main className="relative z-10 pt-28 pb-20 container mx-auto px-4 text-center">
          <h1 className="font-heading text-4xl font-bold mb-4">Товар не найден</h1>
          <Link to="/catalog" className="text-primary hover:underline font-body">
            Вернуться в каталог
          </Link>
        </main>
        <Footer />
      ...
      <main className="relative z-10 pt-28 pb-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            >
              <span className="text-primary font-body text-sm tracking-widest uppercase mb-2">
                {product.category}
              </span>
              ...
              <div className="bg-sand-glass rounded-xl p-6 mb-8 space-y-4">
                <div className="flex justify-between font-body text-sm">
                  <span className="text-muted-foreground">Размер / вес</span>
                ...
                <div className="flex justify-between font-body text-sm">
                  ...
                  <span className="font-medium text-primary">Живой / Охлаждённый</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6 mb-8">
                <span className="font-heading text-4xl font-bold">
                  {product.price} ₽
                  <span className="text-lg text-muted-foreground font-normal">/{product.unit}</span>
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-6 mt-8">
                {[
                  { icon: Truck, text: "Доставка 24ч" },
                  { icon: Thermometer, text: "Холодовая цепь" },
                  { icon: Shell, text: "Свежий улов" },
                ].map((f) => (
          ...
          {related.length > 0 && (
            ...
            >
              <h2 className="font-heading text-3xl font-bold mb-8">
                С этим также <span className="text-gradient-teal">заказывают</span>
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {related.map((rp) => (
                  <Link
                    key={rp.id}
                    to={`/catalog/${rp.id}`}
                    className="block bg-card rounded-xl overflow-hidden border border-border/50 group hover:border-primary/40 transition-all duration-300 hover:-translate-y-1"
                  >
                    ...
                    <div className="p-4">
                      ...
                      <span className="font-body font-bold">
                        {rp.price} ₽<span className="text-sm text-muted-foreground font-normal">/{rp.unit}</span>
                      </span>
                    </div>
                  </Link>
                ))}
